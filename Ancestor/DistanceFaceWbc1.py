...
def get_euler_angle(rotation_vector):
    ...
    w = math.cos(theta)
    x = math.sin(theta)*rotation_vector[0][0]
    y = math.sin(theta)*rotation_vector[1][0]
    z = math.sin(theta)*rotation_vector[2][0]
...
def get_pose_estimation_in_euler_angle(landmark_shape, im_szie):
    try:
        ...
        rotation_vector, translation_vector, camera_matrix, dist_coeffs = get_pose_estimation(im_szie, image_points)
...
while True:
    ...
    for (i, rect) in enumerate(rects):
        ...
        if size[0] > 700:
            h = size[0] / 3
            w = size[1] / 3
            frame = cv.resize( frame, (int( w ), int( h )), interpolation=cv.INTER_CUBIC )
            size = frame.shape
        ...
        cv.putText( frame, euler_angle_str, (0, 120), cv.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 1 )
